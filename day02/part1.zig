...
fn solve(text: []const u8) !u32 {
    ...
    while (lines.next()) |line| {
        ...
        const gameString = games.next() orelse unreachable;
        var gameAndNumber = std.mem.split(u8, gameString, " ");
        _ = gameAndNumber.next() orelse unreachable;
        const gameNumberString = gameAndNumber.next() orelse unreachable;
        const gameNumber = try std.fmt.parseInt(u32, gameNumberString, 10);
        ...
        while (cubes.next()) |cube| {
            ...
            while (colors.next()) |color| {
                ...
                if (std.mem.eql(u8, colorString, "red") and count > 12) {
                    invalidGame = true;
                } else if (std.mem.eql(u8, colorString, "green") and count > 13) {
                    invalidGame = true;
                } else if (std.mem.eql(u8, colorString, "blue") and count > 14) {
                    invalidGame = true;
                }
            }
        }
